const notSupported = obj => () => {
  throw new Error(
    `${obj} not suported in uport-connect core, use full uport-connect library or ConnectCore object`
  );
};
...
const QRUtil = {
  getQRDataURI: notSupported('getQRDataURI'),
  closeQr: notSupported('closeQr'),
  openQr: notSupported('openQr'),
};
